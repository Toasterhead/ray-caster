...
function BOOT()
	renderables={}
	observer=Observer:new(12,4,math.pi/4)
end

function TIC()

	if btn(0) then observer:move(0.1) end
	if btn(1) then observer:move(-0.1) end
	if btn(2) then observer:turn(-0.035) end
	if btn(3) then observer:turn(0.035) end
	if key(17) then observer:move(0.1,-1) end
	if key(23) then observer:move(0.1,1) end
	...
	--	renderables[i+1]=PixelColumn:new(192,i%24,2,i+1,viewport.height-i,math.floor(0.75*i),0,1-(i/viewport.width),((i/viewport.width)+(t/100))*FULL_ROTATION,4)
